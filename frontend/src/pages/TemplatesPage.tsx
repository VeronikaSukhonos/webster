...
import { TextField, SelectField, SelectLabel } from '@components/InputFields';
...
const TemplatesPage = () => {
  ...
  return (
    <div style={{ alignItems: "center", display: "flex", flexDirection: "column" }}>
      ...
      <form onSubmit={searchTemplates} style={{ display: "flex", flexDirection: "row", gap: 5 + "px", marginBottom: 10 + "px" }}>
        <TextField name="search" value={search} onChange={(e/*: ChangeEvent<HTMLInputElement>*/) => setSearch(e.target.value === "" ? undefined: e.target.value)} placeholder="Search template..." />
        <MainButton type="submit" color="white">
          <SearchIcon />
        </MainButton>
        <SelectField name="templateType" value={templateType} onChange={(e) => setTemplateType(e.target.value)} options={templateTypes} />
      </form>
      {auth && <div style={{ display: "flex", flexDirection: "row", gap: 5 + "px", marginBottom: 10 + "px" }}>
        <MainButton onClick={() => setSource(source === "built-in" ? "all" : "built-in")} color={source === "built-in" ? "purple" : "white"}>Built-In</MainButton>
        <MainButton onClick={() => setSource(source === "custom" ? "all" : "custom")} color={source === "custom" ? "purple" : "white"}>Custom</MainButton>
      </div>}
      <ProjectList
          templates={templates}
